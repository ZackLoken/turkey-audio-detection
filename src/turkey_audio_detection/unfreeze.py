...
class Phase:
    n_trainable_top_groups: (
        int  # groups unfrozen, counted from the output side
    )
    lr: float
    batch_size: int
    max_epochs: int
...
def apply_phase(
    model: nn.Module, schedule: UnfreezeSchedule, phase_idx: int
) -> Phase:
