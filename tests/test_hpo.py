...
def test_suggest_config_maps_params_and_keeps_base() -> None:
    ...
    cfg = suggest_config(
        trial, SedTrainConfig(pretrained=False, num_workers=0)
    )
...
def test_run_hpo_smoke(tmp_path, monkeypatch) -> None:
    ...
    for i in range(4):
        ...
        regions = (
            json.dumps([{"label": "Tom", "start_s": 1.0, "end_s": 2.0}])
            if i % 2 == 0
            else "[]"
        )
        rows.append(
    ...
    base = SedTrainConfig(
        ...
        site_map_path="nope.csv",
        seed=0,
        clip_duration_s=3.0,
    )
    study = run_hpo(
        table, tmp_path, base, n_trials=2, storage=str(tmp_path / "study.db")
    )
