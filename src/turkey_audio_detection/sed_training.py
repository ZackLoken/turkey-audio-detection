...
from turkey_audio_detection.sed_data import FrameSedDataset, SedMelParams
...
def site_split(table: pd.DataFrame, cfg: SedTrainConfig) -> pd.DataFrame:
    ...
    if "site_id" not in out.columns:
        out["site_id"] = out.get(
            "aru_id", pd.Series([""] * len(out), index=out.index)
        ).astype(str)
...
def _build_optimizer(
    model: FrameSed, lr: float, backbone_mult: float, weight_decay: float
):
    backbone_params = [
        p
...
def _run_epoch(
    ...
    for log_mel, target, _weak, _ids in loader:
        ...
        if train and mixup_alpha > 0:
            log_mel, target = _batch_mixup(
                log_mel, target, mixup_alpha, mel, rng
            )
    ...
    if not train and any(len(x) for x in yt):
        ...
        for c in range(N_CLASSES):
            ...
            p_, r_, f_ = _binary_prf(
                np.concatenate(yt[c]), np.concatenate(yp[c])
            )
...
def train_sed_from_table(
    ...
    mel = SedMelParams()
    aug = (
        [SpecAugment(rng=np.random.default_rng(cfg.seed + 1))]
        if cfg.specaugment_enabled
        else []
    )
    train_ds = FrameSedDataset(
        train_df, cfg.clip_duration_s, mel, augmentations=aug
    )
    val_ds = (
        FrameSedDataset(val_df, cfg.clip_duration_s, mel)
        if not val_df.empty
        else None
    )
    ...
    schedule = default_schedule(
        ...
        base_lr=cfg.base_lr,
        base_batch=cfg.base_batch_size,
    )
    ...
    for phase_idx, phase in enumerate(schedule.phases):
        ...
        optimizer = _build_optimizer(
            model, phase.lr, cfg.backbone_lr_mult, cfg.weight_decay
        )
...
def train_sed(cfg: SedTrainConfig, project_root: Path) -> dict:
    ...
    table = build_training_table(
        Path(project_root),
        cfg.run_ids,
        include_non_consensus=cfg.include_non_consensus,
    )
    if table.empty:
